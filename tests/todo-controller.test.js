...
describe('sortAndFilterTodos', function () {
    describe('Sorting', function () {
        it('should return descending titles', function () {
            ...
            assert.equal(result.at(0).title, todo3.title, "Title order descending wrong");
        });

        it('should return ascending titles', function () {
            const result = sortAndFilterTodos(todos, "title", false, false);
            assert.equal(result.at(0).title, todo1.title, "Title order ascending is wrong");
        });

        it('should return descending due dates', function () {
            const result = sortAndFilterTodos(todos, "dueDate", true, false);
            assert.equal(result.at(0).dueDate, todo2.dueDate, "Due date descending wrong");
        });

        it('should return ascending due dates', function () {
            const result = sortAndFilterTodos(todos, "dueDate", false, false);
            assert.equal(result.at(0).dueDate, todo1.dueDate, "Due date ascending is wrong");
        });

        it('should return descending importance', function () {
            const result = sortAndFilterTodos(todos, "importance", true, false);
            assert.equal(result.at(0).importance, todo2.importance, "Importance descending wrong");
        });

        it('should return ascending importance', function () {
            const result = sortAndFilterTodos(todos, "importance", false, false);
            assert.equal(result.at(0).importance, todo3.importance, "Importance ascending is wrong");
        });

        it('should return todos even with not existing order criteria', function () {
    ...
    describe('Filter', function () {
        it('should return only open todos', function () {
            ...
            assert.isTrue(result.some(i => i.title === todo2.title), "filter only open todos is wrong");
            assert.isTrue(result.some(i => i.title === todo3.title), "filter only open todos is wrong");
        });
